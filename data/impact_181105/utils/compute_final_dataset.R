...
get_impact_annotated <- function() {
    impact_annotated <- read.table(paste0(data_path, "/annotate_with_click_annotvcf/click_annotvcf_IMPACT_mutations_181105.txt"),
    ...
    impact_vcf <- read.table(paste0(data_path, "/annotate_with_click_annotvcf/all_IMPACT_mutations_181105.vcf"),
...
filter_impact <- function(impact) {
    ...
    # [-823 rows] Hugo_Symbol = CDKN2Ap14ARF and CDKN2A in the tumor sample
...
add_new_features <- function(impact) {
    ...
    # ####################################### Onco_KB
    # 1. Get the raw data
    impact_oncokb <- read.table(paste0(data_path, "/annotate_with_oncokb_final_dataset/oncokb_annotated_final_IMPACT_mutations_181105.txt"),
                                sep = "\t", stringsAsFactors = FALSE, header = TRUE)
...
get_final_dataset <- function() {
    cat("Get raw impact...")
    impact <- read.table(paste0(data_path, "/all_IMPACT_mutations_181105.txt"),
                         sep = "\t", stringsAsFactors = FALSE, header = TRUE)
    cat(" done\n")
    ...
    write.table(impact, paste0(data_path, "/final_IMPACT_mutations_181105.txt"), sep = "\t", row.names = FALSE)
}
...
annotate_final_dataset <- function() {
    ...
    impact <- read.table(paste0(data_path, "/final_IMPACT_mutations_181105.txt"),
                         sep = "\t", stringsAsFactors = FALSE, header = TRUE)
    cat(" done\n")
    ...
    write.table(impact, paste0(data_path, "/annotated_final_IMPACT_mutations_181105.txt"), sep = "\t", row.names = FALSE)
}
